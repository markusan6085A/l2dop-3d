...
for (const e of HUMAN_MYSTIC_SKILL_CATALOG_GENERATED) {
  BY_BATTLE_ID.set(e.battleId, e);
}

export function humanMysticCatalogHasBattleId(battleId: string): boolean {
  return BY_BATTLE_ID.has(canonicalBattleSkillId(battleId));
}

export function humanMysticCatalogEntry(
  battleId: string
