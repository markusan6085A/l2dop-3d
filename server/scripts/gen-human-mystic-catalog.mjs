/**
 * Каталог Human Mystic з text-rpg → `humanMysticSkillCatalog.generated.ts`.
 * Запуск з кореня репо: `npm run gen:hm-skills`
 */
import fs from 'fs';
...
const MAGE = new Set(['human_mage']);
const WIZARD_CHAIN = new Set([
  'human_wizard',
  'human_sorcerer',
  'human_necromancer',
  'human_warlock',
...
const ALL_MYSTIC = new Set([...MAGE, ...WIZARD_CHAIN, ...CLERIC_CHAIN]);

/** Підпапка text-rpg → набір l2Profession (l2dop-3d). */
const FOLDER_PROFS = {
  common: ALL_MYSTIC,
  Wizard: WIZARD_CHAIN,
  Cleric: CLERIC_CHAIN,
  Sorcerer: new Set(['human_sorcerer', 'human_archmage']),
  Necromancer: new Set(['human_necromancer', 'human_soultaker']),
  Warlock: new Set(['human_warlock', 'human_arcana_lord']),
  Bishop: new Set(['human_bishop', 'human_cardinal']),
  Prophet: new Set(['human_prophet', 'human_hierophant']),
  Archmage: new Set(['human_archmage']),
  Soultaker: new Set(['human_soultaker']),
  ArcanaLord: new Set(['human_arcana_lord']),
  Cardinal: new Set(['human_cardinal']),
  Hierophant: new Set(['human_hierophant']),
};

/** Українські назви (Interlude / узгоджено з l2); для відсутніх id — «Скіл №id». */
const NAME_UK = {
  118: 'Рух мага',
  146: 'Антимагія',
...
function folderOf(filePath) {
  const rel = path.relative(TEXT_HM, filePath);
  return rel.split(path.sep)[0] || 'common';
}

function parseLevels(s) {
  const levelsMatch = s.match(/levels:\s*\[([\s\S]*?)\]\s*(?:,|\n\s*\})/);
  if (!levelsMatch) return [];
  const block = levelsMatch[1];
  const rowRe = /\{([\s\S]*?)\}/g;
  const out = [];
  let row;
  while ((row = rowRe.exec(block)) !== null) {
    const chunk = row[1];
    const level = Number(
      chunk.match(/["']?level["']?\s*:\s*(-?\d+(?:\.\d+)?)/)?.[1]
    );
    const requiredLevel = Number(
      chunk.match(/["']?requiredLevel["']?\s*:\s*(-?\d+(?:\.\d+)?)/)?.[1]
    );
    const spCost = Number(
      chunk.match(/["']?spCost["']?\s*:\s*(-?\d+(?:\.\d+)?)/)?.[1]
    );
    const mpCost = Number(
      chunk.match(/["']?mpCost["']?\s*:\s*(-?\d+(?:\.\d+)?)/)?.[1]
    );
    const power = Number(
      chunk.match(/["']?power["']?\s*:\s*(-?\d+(?:\.\d+)?)/)?.[1]
    );
    if (
...
function parseEffects(s) {
  ...
  const out = [];
  const re =
    /\{\s*["']?stat["']?\s*:\s*"([^"]+)"\s*,\s*["']?mode["']?\s*:\s*"([^"]+)"(?:\s*,\s*["']?value["']?\s*:\s*(-?\d+(?:\.\d+)?))?/g;
  let x;
...
for (const filePath of walkTs(TEXT_HM)) {
...
export const HUMAN_MYSTIC_ACTIVE_L2_IDS: readonly number[] = [${activeIds.join(', ')}];
`;
