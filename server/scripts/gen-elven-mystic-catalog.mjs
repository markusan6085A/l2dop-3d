/**
 * Каталог Elven Mystic з text-rpg → `elvenMysticSkillCatalog.generated.ts`.
 * Запуск з кореня репо: `npm run gen:em-skills`
 */
import fs from 'fs';
...
const ELF_MAGE = new Set(['elf_mage']);
/** Гілка чарівника: від Elven Wizard до Mystic Muse / Elemental Master. */
...
/** Гілка жреця: Oracle → Elder → Eva's Saint. */
const CLERIC_TRACK = new Set([
  'elf_elven_oracle',
  'elf_elven_elder',
  'elf_evas_saint',
]);
const ALL_ELF_MYSTIC = new Set([
  ...ELF_MAGE,
  ...WIZARD_TRACK,
  ...CLERIC_TRACK,
]);

/** Не експортувати в каталог (сміття/дубль з датапаку). */
const SKIP_IDS = new Set([1320]);

/** Довгі підказки окремо від назви. */
const HINT_UK = {
...
const FOLDER_PROFS = {
  common: ALL_ELF_MYSTIC,
  ElvenWizard: WIZARD_TRACK,
  ElementalSummoner: new Set(['elf_elemental_summoner', 'elf_elemental_master']),
  ElementalMaster: new Set(['elf_elemental_master']),
  Spellsinger: new Set(['elf_spellsinger', 'elf_mystic_muse']),
  MysticMuse: new Set(['elf_mystic_muse']),
  ElvenOracle: CLERIC_TRACK,
  ElvenElder: new Set(['elf_elven_elder', 'elf_evas_saint']),
  EvasSaint: new Set(['elf_evas_saint']),
};

/** Українські назви (спільні з людиною-магом де id збігаються). */
const NAME_UK = {
  118: 'Рух мага',
  146: 'Антимагія',
...
function folderOf(filePath) {
  const rel = path.relative(TEXT_EM, filePath);
...
function mergeProfSets(folders) {
  ...
  for (const f of folders) {
    const st = FOLDER_PROFS[f] ?? ALL_ELF_MYSTIC;
...
for (const filePath of walkTs(TEXT_EM)) {
...
export const ELVEN_MYSTIC_ACTIVE_L2_IDS: readonly number[] = [${activeIds.join(', ')}];
`;
