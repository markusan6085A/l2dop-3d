/**
 * Одноразова міграція S-grade itemId у PostgreSQL.
 *
 *   npx tsx server/scripts/migrate-s-weapon-item-ids-v1.ts --dry-run
 *   CONFIRM_S_WEAPON_ID_MIGRATION_V1=1 npx tsx server/scripts/migrate-s-weapon-item-ids-v1.ts --apply
 *
...
function printStaticMigrationReport(): void {
  console.log('\n=== S weapon itemId migration (static plan) ===');
  console.log('Legacy map entries:', Object.keys(LEGACY_S_WEAPON_ID_MAP).length);
  for (const from of LEGACY_S_WEAPON_SOURCE_IDS.sort((a, b) => a - b)) {
    console.log(`  ${from} → ${LEGACY_S_WEAPON_ID_MAP[from]}`);
  }
  console.log('\nEvent legacy ids:', LEGACY_S_WEAPON_EVENT_IDS.join(', '));
  const preflight = preflightSWeaponMigration();
  if (preflight.issues.length > 0) {
    console.log('\nPreflight issues:');
    for (const issue of preflight.issues) console.log(`  - ${issue}`);
  } else {
    console.log('\nPreflight: OK (no event ambiguity, no cross-grade collisions)');
  }
...
async function readMarker(): Promise<string | null> {
  const row = await prisma.serverMeta.findUnique({
    where: { key: S_WEAPON_ITEM_ID_MIGRATION_MARKER },
  });
...
async function main(): Promise<void> {
  ...
  if (!dryRun && !apply) {
    console.error('Usage: --dry-run | --apply (with CONFIRM_S_WEAPON_ID_MIGRATION_V1=1)');
    process.exit(1);
  }
  ...
  if (marker) {
    console.error(`Migration already applied (marker ${S_WEAPON_ITEM_ID_MIGRATION_MARKER}=${marker})`);
    process.exit(1);
  }

  const preflight = preflightSWeaponMigration();
  if (preflight.eventAmbiguity || preflight.crossGradeCollision) {
    console.error('\nRefusing migration due to preflight failure:');
    for (const issue of preflight.issues) console.error(`  - ${issue}`);
    process.exit(1);
  }

  if (apply && process.env.CONFIRM_S_WEAPON_ID_MIGRATION_V1 !== '1') {
    console.error('Refusing --apply without CONFIRM_S_WEAPON_ID_MIGRATION_V1=1');
    process.exit(1);
  }
  ...
  for (const row of characters) {
    ...
    mergeCounts(legacyCounts, collectItemIdsFromInventory(inv).filter((r) => r.itemId in LEGACY_S_WEAPON_ID_MAP));
    ...
    mergeCounts(
      legacyCounts,
      collectItemIdsFromWarehouse(wh.stacks).filter((r) => r.itemId in LEGACY_S_WEAPON_ID_MAP),
    );
  ...
  for (const listing of marketListings) {
    const nextId = mapLegacySWeaponItemId(listing.itemId);
  ...
  console.log('\n=== S weapon itemId migration dry-run ===\n');
  ...
  if (legacyCounts.size === 0) {
    ...
    for (const [id, count] of [...legacyCounts.entries()].sort((a, b) => a[0] - b[0])) {
      console.log(`  ${id} × ${count} → ${mapLegacySWeaponItemId(id)}`);
    }
  }
  console.log('\nExpected remap map:');
  for (const from of LEGACY_S_WEAPON_SOURCE_IDS.sort((a, b) => a - b)) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.serverMeta.upsert({
      where: { key: S_WEAPON_ITEM_ID_MIGRATION_MARKER },
      create: {
        key: S_WEAPON_ITEM_ID_MIGRATION_MARKER,
